...
float temp_CORE_floor = 45;	// coretemp "Package id 0" - not part of applesmc,
float temp_CORE_ceiling = 55;	// added since applesmc has no visibility into CPU die temp
...
void read_cfg(char* name)
{
	...
	if(fp != NULL)
	{
		...
		temp_CORE_ceiling = read_param("temp_CORE_ceiling",	0, 90, 55);
		temp_CORE_floor = read_param("temp_CORE_floor",		0, temp_CORE_ceiling - 1, 45);
	...
	printf("\ttemp_CORE_floor: %.0f\n", temp_CORE_floor);
	printf("\ttemp_CORE_ceiling: %.0f\n", temp_CORE_ceiling);
