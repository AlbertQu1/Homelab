...
#define CORETEMP_ID		"coretemp"
#define CORE_PKG_LABEL	"Package id 0"
...
// coretemp is a separate hwmon chip from applesmc: it carries the CPU's own
// digital die sensor ("Package id 0"), which applesmc has no visibility into.
// Tracked independently of the sensors[] array/scan_sensors(), which is
// applesmc-only.
char core_pkg_fname[PATH_MAX];
int core_pkg_found = 0;
float core_pkg_value = 0;
...
//------------------------------------------------------------------------------
// Locate the coretemp hwmon device and its "Package id 0" sensor.
// Unlike find_applesmc(), this is non-fatal: not every machine has coretemp
// (e.g. non-Intel), and the daemon should keep running on applesmc alone.

void find_coretemp()
{
	DIR *fd_dir;
	int ret;
	char coretemp_path[PATH_MAX];
	...
	if(fd_dir != NULL)
	{
		...
		while((dir_entry = readdir(fd_dir)) != NULL && coretemp_path[0] == 0)
		{
			if(dir_entry->d_name[0] != '.')
			{
				char name_path[PATH_MAX];
				...
				// unlike applesmc, coretemp exposes "name" directly at the
				// hwmon dir root, not under device/
				...
				if(fd_name > -1)
				{
					char name[sizeof(CORETEMP_ID)];

					ret = read(fd_name, name, sizeof(CORETEMP_ID) - 1);

					close(fd_name);

					if(ret == sizeof(CORETEMP_ID) - 1)
					{
						if(strncmp(name, CORETEMP_ID, sizeof(CORETEMP_ID) - 1) == 0)
						{
							char *dev_path;
							char *last_slash = strrchr(name_path, '/');
							...
							if(last_slash != NULL)
							{
								*last_slash = 0;
								...
								if(dev_path != NULL)
								{
									strncpy(coretemp_path, dev_path, sizeof(coretemp_path) - 1);
									coretemp_path[sizeof(coretemp_path) - 1] = 0;
									free(dev_path);
								}
							}
						}
	...
	if(coretemp_path[0] == 0)
	{
		printf("Warning: no coretemp device found, CORE fan control disabled\n");
		return;
	}

	// find the "Package id 0" sensor among coretemp's tempN_input files
	...
	{
		char fname[PATH_MAX];
		char label_fname[PATH_MAX];

		sprintf(fname, "%s/temp%d_input", coretemp_path, count);
		...
		sprintf(label_fname, "%s/temp%d_label", coretemp_path, count);

		FILE *fp = fopen(label_fname, "r");
		if(fp != NULL)
		{
			char label[SENSKEY_MAXLEN * 2];
			memset(label, 0, sizeof(label));
			int n = fread(label, 1, sizeof(label) - 1, fp);
			fclose(fp);

			if(n > 0)
			{
				char *p_endl = strrchr(label, '\n');
				if(p_endl)
				{
					*p_endl = 0;
				}

				if(strcmp(label, CORE_PKG_LABEL) == 0)
				{
					strncpy(core_pkg_fname, fname, sizeof(core_pkg_fname) - 1);
					core_pkg_fname[sizeof(core_pkg_fname) - 1] = 0;
					core_pkg_found = 1;
					break;
				}
			}
	...
	if(core_pkg_found)
	{
		printf("Found coretemp at %s (%s)\n", coretemp_path, core_pkg_fname);
	}
	else
	{
		printf("Warning: coretemp found at %s but no \"%s\" sensor, CORE fan control disabled\n",
			   coretemp_path, CORE_PKG_LABEL);
	}
}
...
void read_coretemp()
{
	...
	int fd = open(core_pkg_fname, O_RDONLY);
	if(fd < 0)
	{
	...
	core_pkg_value = (float)atoi(val_buf) / 1000.0;
}
...
void calc_fan()
{
	...
	// calc fan speed for coretemp "Package id 0" (CPU die) - applesmc has no
	// sensor for this, so without it the fan never reacts to real CPU heat

	if(core_pkg_found)
	{
		float temp_window = temp_CORE_ceiling - temp_CORE_floor;
		float normalized_temp =(core_pkg_value - temp_CORE_floor) / temp_window;
		float fan_CORE_speed =(normalized_temp * fan_window);
		if(fan_CORE_speed > fan_speed)
		{
			fan_speed = fan_CORE_speed;
			fan_ctl = CTL_CORE;
		}
	}
